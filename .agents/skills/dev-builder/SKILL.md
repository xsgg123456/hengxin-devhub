...
[第一性原则]
    ...
    母版优先：用户或上游文档指定既有可运行前端时，它是代码实现基线，不是截图参考。直接在母版原文件上最小改造，保留其布局、组件、交互和状态逻辑；禁止另建平行壳、重写同等基础设施或自造命名空间 override。只有列出具体候选及其无法满足 Spec 的证据并获得用户明确同意，才允许新建替代实现。
...
[开发规则清单]
    ...
    [视觉与复用]
        - 每个 UI Task 编码前先读 Design-Brief，并输出“需求 → 原位修改的既有文件/组件 → 必须保留的交互和状态逻辑”映射；有指定母版却给不出具体源码路径时不得开始编码
        - 编码从映射中的母版文件开始，不先搭新壳再模仿。新增页面或组件前列出已检索的既有候选；能最小改造就不得新建，新建时必须记录每个候选无法满足 Spec 的事实证据
        - 新页面继承相邻同类页面的语义、交互、危险操作规则，不一页单造一套
        - 完成时用上述映射、Git diff 和真实渲染对比证明复用；只说“风格一致”或只列组件名不算证据
...
[Phase 执行流程]
    ...
    每个 Task 走 review→fix 循环：
        ...
        编码后自检：代码实际值对照设计数值，行为对照 Spec；有母版时同时核对复用映射，确认没有平行实现同等能力
...
[初始化模式]
    ...
    - 用户或 DEV-PLAN 指定既有前端母版时，开工前在 `.codex/project-checks.json` 登记 `reuseBaseline`：允许的前端根、不得删除的母版关键路径和用于识别平行前端根的入口模式；登记完成后先移植或原位使用母版，再写业务改动
