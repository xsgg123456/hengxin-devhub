...
[调试标准]
    ...
    修复：一次一个逻辑点 → 执行 DEV-PLAN 针对当前技术栈声明的验证命令 → 功能验证 bug 不再复现 → 回归验证相关功能正常。修复失败就回退重新假设，连续 3 次失败停下重审。
    进程：bug 涉及端口占用时先查 PID、命令行、工作目录和父进程，确认属于当前项目才终止；归属不明就报告用户，不得按固定端口无差别强杀。
