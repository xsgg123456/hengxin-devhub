...
| 用途 | 工作目录 | 命令 | 何时执行 |
|------|----------|------|----------|
| 类型或编译检查 | `[项目子目录]` | `[按技术栈填写真实命令]` | 每个 Task 提交前 |
| 单元与集成测试 | `[项目子目录]` | `[真实命令]` | 每个 Task 和 Phase 完成时 |
| E2E 或功能测试 | `[项目子目录]` | `[真实命令]` | 每个 Phase 完成时 |
| 构建 | `[项目子目录]` | `[真实命令]` | 每个 Phase 完成时 |
...
5. **技术栈表**：
   ...
   - 说明列写选择理由或用途。
   - 验证命令必须与技术栈一致，不得给 Python、Go、Rust 等项目硬塞 tsc 或 npm。
6. **数据库表**：
   - 标注在哪个 Phase 创建。
