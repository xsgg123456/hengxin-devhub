...
beforeAll(async()=>{
  ...
  app=(await buildApp({...env,DINGTALK_CLIENT_ID:'test-client',DINGTALK_CLIENT_SECRET:'test-secret',DINGTALK_CORP_ID:'test-corp',DINGTALK_REDIRECT_URI:'http://127.0.0.1:4322/api/auth/dingtalk/callback'},{db,logging:false,dingClient:client})).app
  await app.ready()
})
afterAll(async()=>{
...
it('扫码回调绑定Cookie并一次性消费state，真实创建服务端会话',async()=>{
  const {state,cookie}=await start()
  const callback=()=>app.inject({url:`/api/auth/dingtalk/callback?state=${state}&code=temporary`,headers:{cookie}})
...
it('错误Cookie、过期state和停用用户都不能取得登录会话',async()=>{
  const {state,cookie}=await start()
  const invalid=await app.inject(`/api/auth/dingtalk/callback?state=${state}&code=x`)
  ...
  expect((await app.inject({url:`/api/auth/dingtalk/callback?state=${state}&code=x`,headers:{cookie}})).headers.location).toContain('authorization_failed')
