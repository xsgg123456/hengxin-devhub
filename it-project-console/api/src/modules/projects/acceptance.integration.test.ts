...
describe('业务确认验收真实API', () => {
  ...
  await db.project.update({ where: { id }, data: { demandId: demand.id, businessOwnerId: business } })
    ...
    expect(completed.acceptanceHistory).toEqual(expect.arrayContaining([expect.objectContaining({ action: 'submit', round: 1, summary: '首轮交付', url: 'http://192.168.1.10:8080/result' }), expect.objectContaining({ action: 'return', actorId: business, summary: '缺少场景' })]))
