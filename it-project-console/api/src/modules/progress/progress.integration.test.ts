...
describe('先排期后执行的真实事务', () => {
  ...
  it('业务通过完成且不归档，重开清实际结果，重做保留原完成历史', async () => {
    const id = await create(), url = `/api/projects/${id}/progress`
    expect((await schedule(id)).statusCode).toBe(200)
    for (let version = 2; version <= 5; version++) expect((await call(url, overall(version, { status: 'completed' }))).statusCode).toBe(200)
    const acceptance = (version: number, action: string, user = owner, extra = {}) => call(`/api/projects/${id}/acceptance`, { requestId: key(), version, action, summary: '验收说明', ...extra }, user)
    expect((await call(url, overall(6, { status: 'completed' }))).statusCode).toBe(400)
    expect((await acceptance(6, 'assign', manager, { ownerId: business })).statusCode).toBe(200)
    expect((await acceptance(7, 'submit')).statusCode).toBe(200)
    expect((await acceptance(8, 'accept', business)).statusCode).toBe(200)
    const completed = await row(id)
    expect(completed).toMatchObject({ status: 'COMPLETED', archived: false, version: 9 })
    expect(completed.actualCompletedAt).toBeInstanceOf(Date)
    expect((await call(url, overall(9))).statusCode).toBe(409)
    const previous = await db.stageHistory.findFirstOrThrow({ where: { projectId: id, stage: '验收交付', completedAt: { not: null } } })
    expect((await action(id, 9, 'reopen', owner)).statusCode).toBe(403)
    expect((await action(id, 9, 'reopen')).statusCode).toBe(200)
    expect(await row(id)).toMatchObject({ actualCompletedAt: null, acceptanceStatus: 'none' })
    expect((await acceptance(10, 'submit')).statusCode).toBe(200)
    expect((await acceptance(11, 'accept', business)).statusCode).toBe(200)
    ...
    expect(await db.notificationOutbox.count({ where: { projectId: id, eventType: 'PROJECT_COMPLETED' } })).toBe(4)
  })
