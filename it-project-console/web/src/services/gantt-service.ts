...
export function buildGanttRows(
  ...
  return [...projects]
    ...
    .flatMap((project): GanttRow[] => {
      if (!filters.includeArchived && (project.archived || project.status === 'cancelled'))
        return []
      ...
      const risks =
        project.riskVersion !== undefined
          ? project.risks
          : computeProjectRisks(project, changes, now)
