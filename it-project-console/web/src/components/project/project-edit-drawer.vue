    ...
    <template v-if="form">
      ...
      <ElForm label-position="top" :disabled="busy" class="edit-form">
        ...
        <section v-show="tab === 'people'">
          <div class="edit-grid"><ElFormItem label="业务负责人"><ElSelect v-model="form.businessOwnerId" filterable clearable aria-label="业务负责人"><ElOption v-for="u in users" :key="u.id" :label="u.name + ' · ' + u.department" :value="u.id" /></ElSelect></ElFormItem><ElFormItem label="业务验收人"><ElSelect v-model="form.acceptanceOwnerId" filterable clearable aria-label="业务验收人"><ElOption v-for="u in users.filter(person => person.role === 'business')" :key="u.id" :label="u.name + ' · ' + u.department" :value="u.id" /></ElSelect></ElFormItem></div>
