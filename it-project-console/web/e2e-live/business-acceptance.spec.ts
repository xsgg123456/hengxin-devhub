...
test('真实业务验收：管理指定、工程师提交撤回重提、业务退回整改后通过、刷新持久化', async ({ page }) => {
  ...
  await detail.getByLabel('交付访问链接（选填）', { exact: true }).fill('192.168.1.10:8080/acceptance')
  ...
  await detail.getByLabel('交付访问链接（选填）', { exact: true }).fill('http://intranet:8080/acceptance')
  ...
  await detail.getByLabel('交付访问链接（选填）', { exact: true }).fill('www.baidu.com')
  ...
  expect(saved.acceptanceUrl).toBe('www.baidu.com')
  await expect(detail.getByRole('link', { name: 'www.baidu.com', exact: true }).first()).toHaveAttribute('href', 'http://www.baidu.com/')
  await expect(detail.getByRole('link', { name: 'http://intranet:8080/acceptance', exact: true })).toHaveAttribute('href', 'http://intranet:8080/acceptance')
