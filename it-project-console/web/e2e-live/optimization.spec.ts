...
test('真实优化需求经审批接单后仅一节点，计划保存并业务验收，刷新保留关联', async ({ page }) => {
  ...
  await proposal.getByRole('button', { name: '确认接单优化', exact: true }).click()
  ...
  await identity(page, '陈立峰')
  await openAcceptance(page, child.id)
  await acceptanceDetail(page).getByRole('button', { name: '编辑优化', exact: true }).click()
  const fullEditor = page.getByRole('dialog', { name: '编辑优化', exact: true })
  await fullEditor.getByRole('tab', { name: '阶段与日期', exact: true }).click()
  await fullEditor.locator('input[aria-label$="计划交付"]').fill('2099-10-21')
  await fullEditor.locator('input[aria-label$="计划交付"]').press('Tab')
  ...
  await confirmation.getByLabel('修改原因').fill('核对优化完成验收日期，验证单节点里程碑同步')
  ...
  await expect(acceptanceDetail(page)).toContainText('待验收')
  await identity(page, '李思敏')
  await openAcceptance(page, child.id)
  ...
  await select(page, page.locator('main'), '项目类型', '项目优化')
