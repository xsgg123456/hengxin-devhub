...
test('真实多格式多文件失败重试→提交→跨角色立项→刷新持久与审批隔离', async ({ page }) => {
  ...
  const createdDemand = createdWorkspace.data.database.demands.find((item: { name: string }) => item.name === name)
  expect(createdDemand.code).toMatch(/^XQ-\d{4}-\d{4,}$/)
  const submittedMessage = page.locator('.el-message--success').filter({ hasText: '已提交，等待管理人员评估' })
