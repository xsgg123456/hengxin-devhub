...
async function switchUser(page: Page, name: string) {
  const currentUser = page.getByRole('button', { name: '当前用户' })
  if ((await currentUser.textContent())?.includes(name)) return
  const account = page.getByRole('button', { name: new RegExp(name + ' ·') })
  async function selectAccount() {
    if (!(await account.isVisible())) await currentUser.click()
    const [response] = await Promise.all([
      page.waitForResponse(
        (response) =>
          new URL(response.url()).pathname === '/api/auth/dev-login' &&
  ...
  let response = await selectAccount()
  if (response.status() === 429) {
    const seconds = Number(response.headers()['retry-after'])
    expect(seconds).toBeGreaterThan(0)
    expect(seconds).toBeLessThanOrEqual(60)
    // 保留真实登录限流，按服务端窗口等待后再次操作相同界面按钮。
    test.setTimeout(test.info().timeout + seconds * 1000 + 1000)
    await page.waitForTimeout(seconds * 1000 + 100)
    response = await selectAccount()
  }
  expect(response.ok()).toBeTruthy()
  await expect(currentUser).toContainText(name)
}
...
test('真实多格式多文件失败重试→提交→跨角色立项→刷新持久与审批隔离', async ({ page }) => {
  ...
  await drawer.locator('input[type=file]').setInputFiles([
    {
      ...
      buffer: Buffer.from('design-file-fixture')
    },
    {
      name: '需求清单.xlsx',
  ...
  expect(createdDemand.code).toMatch(/^XQ-\d{4}-\d{4,}$/)
  const submittedMessage = page
    .locator('.el-message--success')
    .filter({ hasText: '已提交，等待管理人员评估' })
  await expect(submittedMessage).toContainText(createdDemand.code)
  await expect(submittedMessage).not.toContainText(createdDemand.id)
  await expect(row(page, name)).toContainText(createdDemand.code)
  await expect(row(page, name)).not.toContainText(createdDemand.id)
  await row(page, name).getByRole('button', { name: '查看', exact: true }).click()
  const demandDetail = page.getByRole('dialog', { name: `需求详情 · ${name}` })
  ...
  await expect(drawer.locator('.el-date-editor')).toHaveCount(1)
  await expect(drawer.getByLabel('审批确认上线日期', { exact: true })).toHaveValue('2099-12-31')
  ...
  await page
    ...
    .click()
  const proposalDrawer = page.getByRole('dialog', { name: '接单详情 · ' + name })
  await proposalDrawer.getByRole('button', { name: '确认接单并立项' }).click()
  await expect(proposalDrawer).not.toBeVisible()
  ...
  const demand = workspace.data.database.demands.find(
    (item: { name: string }) => item.name === name
  )
  expect(demand.code).toBe(createdDemand.code)
