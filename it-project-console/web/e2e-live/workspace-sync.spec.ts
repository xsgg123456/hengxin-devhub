...
test('双账号自动同步、中文校验、保留筛选和旧编辑版本，SSE断开后轻量检查补同步', async ({ page: manager, browser }) => {
  ...
  try {
    ...
    await select(manager, manager.locator('main'), '需求类型', '项目优化')
    ...
    await expect(manager.getByText('正式项目 0 条 · 项目优化 1 条 · 合计 1 条', { exact: true })).toBeVisible()
    expect(await table!.evaluate(element => element.isConnected)).toBe(true)
    await expect(manager.locator('.el-select').filter({ hasText: '项目优化' })).toHaveCount(1)
