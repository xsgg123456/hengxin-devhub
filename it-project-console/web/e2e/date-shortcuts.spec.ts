...
test('上海快捷日期、编号搜索、原筛选清空和需求联动', async ({ page }) => {
  ...
  await page.goto('/')
  await expect(page.getByRole('button', { name: '直接创建项目' })).toBeVisible({ timeout: 15_000 })
  ...
  await expect(cards).toHaveCount(3)
  await page.getByRole('menuitem', { name: '需求池', exact: true }).click()
  await expect(page.getByRole('heading', { name: '我的需求 / 需求池', exact: true })).toBeVisible()
