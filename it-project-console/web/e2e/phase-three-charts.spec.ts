...
test('搜索筛选与图表明细同源，刷新保留筛选，图表明细进入共享详情', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '正式项目 · 客户数据治理一期 · 王浩然', exact: true }).click()
  await expect(page.getByRole('dialog', { name: '项目详情', exact: true })).toContainText('七环节计划与执行')
...
test('业务点击待立项定位只读需求，需求双分布与趋势随表格筛选', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '待审批 / 待接单 1 个', exact: true }).click()
  ...
  await expect(page.getByRole('button', { name: '立项审批', exact: true })).toHaveCount(0)
  ...
  await page.locator('.el-select').filter({ has: page.getByRole('combobox', { name: '完成情况', exact: true }) }).click()
  await page.getByRole('option', { name: '正常完成', exact: true }).click()
