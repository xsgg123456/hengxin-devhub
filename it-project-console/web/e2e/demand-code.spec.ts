...
test('旧需求补号，新需求创建后列表与详情一致，刷新编辑保持编号', async ({ page }) => {
  ...
  await expect(editor.getByLabel('需求首次提出日期', { exact: true })).toHaveCount(0)
  ...
  await expect(edit.getByLabel('需求首次提出日期', { exact: true })).toHaveCount(0)
  await edit.getByLabel('这次要解决什么问题（一句话）').fill('编辑后保持原有需求编号')
  await edit.getByRole('button', { name: '重新提交', exact: true }).click()
  await expect(edit).not.toBeVisible()
  expect((await snapshot(page)).database.demands.find(d => d.id === demand.id)?.firstRequestedOn).toBe(demand.firstRequestedOn)
  await page.reload()
  await expect(row(page, name)).toContainText(demand.code!)
