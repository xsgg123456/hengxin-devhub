...
更新期间有短暂停机，健康检查通过才把版本标记为成功。发布包保留策略见下文：默认保留当前成功版和上一成功版；不删除 Docker 镜像或数据库/对象存储备份，不能使用全局 Docker prune 代替本项目清理。

### 发布包保留策略（2026-09-11）

- `pnpm release -Action Deploy` 成功后执行 `prune-releases.sh`，在发布锁内核对 current 未变化，保护 current、previous。pending 存在、指针损坏、路径异常或有其他发布操作时拒绝清理。
- 成功部署会在版本目录写入 `.deployed` 标记。自动删除仅针对带此标记的过期版本：删除服务器 `releases/<版本>` 及对应 `incoming/itpc-<版本>.tar.gz`，然后移除本机同名 `output/releases/<版本>`。只有打包或上传不会触发清理。
- 未部署/失败包不自动删；需要保留的版本可在其目录放 `PIN`（服务器和本机分别生效）。因此通常保留两个成功版本，待发布、失败和 PIN 包是额外保留项。
- 上传前在同一发布锁内登记 `incoming/.uploading-<版本>`，成功部署后移除；清理保护该标记，避免并发重新上传历史版本时被删除。中断上传的标记保留，需人工核实后处理。
- 历史无标记包只能按人工核对的明确清单清理，不能按目录日期推断正在使用的版本。原始直接调用 `deploy-bundle.sh` 不负责清理，统一从 PowerShell 发布入口执行收尾。
- 清理失败只报告警告，不把已经健康的部署改成失败，不执行数据库恢复；可用当前包内 `prune-releases.sh plan|apply /opt/it-project-console <current版本>` 重试服务器清理。本机断线后遗留包需下次人工核对清理。
- 数据库备份继续采用独立策略：每日完整备份保留14个成功批次及 PIN 批次；部署前SQL备份不由发布包清理处理。上一版本包不保证跨数据库迁移自动回退。
- 策略修改随下一次正常发布生效，本次不会为清包重新部署业务；本次运维清理与保留版本记录见 `RELEASE-RETENTION-20260911.md`。
