...
| 当前可执行阶段 | Phase 9 技术验证通过；企业真实联调待配置，随后进入 Phase 10 |
...
**状态**：部署目标已确认（2026-09-09）；统一发布脚本、生产镜像构建与本地隔离安装验证已完成，尚未向目标服务器部署或切换入口；Phase 9 企业真实联调、Phase 10 安全整改与最终验收仍待完成。
...
- 当前仅确认目标和只读核对代理，尚未改动服务器配置、停止旧应用或发送真实消息。

**统一发布脚本任务（2026-09-09）**：用户要求以后统一从本机打包上传这台服务器，本轮交付脚本并验证，不执行旧系统入口切换。

发布门禁：本轮允许生成带完整审计记录的本地验证包；正式 `/opt/it-project-console` 部署必须审计 high/critical 为零。既有 Vite/xlsx 告警仍由 Phase 10 安全整改消除，不能绕过门禁上线。本机隔离测试根仅验证发布流程，不表示生产准入。
