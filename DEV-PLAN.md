> 2026-09-15 通知修订执行：1 同步需求规则；2 在现有提交事务新增审批通知、原通知模板按主负责人显示制定计划提示；3 隔离验证草稿/提交/重试/修改/撤回重提、审批人范围、过期跳过与不同收件人文案；4 类型、构建、集成测试和独立审查通过后提交。
> 2026-09-14 内网交付链接修订：1 同步验收第3条；2 原位调整前后端校验与详情/历史链接解析，保留草稿和幂等；3 覆盖HTTP/IP/主机名/无协议及危险协议拒绝，执行类型/单测/隔离集成/浏览器/构建与独立审查；4 本地提交后沿用本轮生产发布流程更新。
> 2026-09-14 本地实现与回归完成：[业务确认验收](it-project-console/docs/BUSINESS-ACCEPTANCE-ITERATION.md)。工程师提交、指定业务负责人通过或退回；通过后自动完成项目。类型/构建、281单测、134隔离集成、49原型浏览器及全部真实浏览器文件通过，详见[验证记录](it-project-console/docs/BUSINESS-ACCEPTANCE-VERIFICATION.md)。本轮覆盖旧工程师自行完成验收规则，生产部署另需授权。
...
## 2026-09-16 素材接入与生产发布

1. 校验用户视频和 ZIP，复制原文件到版本化静态目录，记录大小与 SHA256；不执行包内指令。
2. 在既有需求页传入资源地址；布局和播放/下载组件不重建。补真实播放、关闭释放、下载文件名与字节一致性、失败重试验证。
3. 独立审查、本地提交；使用既有发布脚本完成测试、依赖与产物审计、镜像打包和校验上传。
4. 备份后部署，线上验证服务、资源哈希、视频分段响应及浏览器播放；失败按迁移兼容约束回退。保留当前和上个成功版本，不删除业务数据、不校正历史名称。

## 2026-09-16 正式开发计划

1. 同步 [本轮规则](it-project-console/docs/DEMAND-EXPERIENCE-ITERATION.md)，原位复用页面、需求编辑抽屉和设备/布局判断。
2. 名称在项目编辑/接单重新评估事务中同步需求并审计、递增版本；补充幂等、回滚及关联范围测试，准备指定存量记录校正方案，不发布生产。
3. 前端改文件名、优化提交入口和可搜索原项目字段；保留草稿/附件/并发/未保存提醒，空原项目不可保存为正式需求。
4. 电脑小窗口不销毁业务页面；折叠菜单、内部滚动、抽屉适配；真实浏览器验证缩放中表单保留。
5. 执行前后端类型、单测、隔离集成、构建、浏览器回归与独立审查；真实视频/ZIP缺失不伪造获取成功。

## 2026-09-16 需求提交通道资源预览

### 目标

在现有业务需求页面的“需求提交通道”中，增加视频教程与 Skill 成品包的前端展示和获取入口，保持原有提交流程不变。

### 执行步骤与完成标准

1. 同步 Product-Spec、Design-Brief 与变更记录：明确资源区只展示/获取，业务自愿使用；资源未配置时不阻塞提交。
2. 基于 `it-project-console/web/src/views/my-demands/index.vue` 原位接入独立资源组件：复用现有卡片、按钮、图标和 Dialog；真实视频/zip 通过资源地址配置，预览无资源时显示待上传占位。
3. 增加前端交互验证：业务人员可见资源区，视频打开弹窗，未配置资源不产生假成功，原有正式需求提交按钮仍可用。
4. 完成 `typecheck`、原型构建、自动化测试和真实浏览器截图；不连接生产、不部署。

# Development Plan — IT 项目管理台

2026-09-11 发布包整理：1 核对 current/previous/pending 和明确历史清单；2 实现带发布锁、路径校验、PIN 与成功标记保护的清理，接入成功发布后的收尾；3 隔离测试正常、失败、并发锁及路径逃逸，独立审查；4 按清单清理本机与服务器历史包，核验保留版本、校验和及线上健康。只清发布产物，不删除镜像、数据库备份或运维目录，不重新部署业务。
...
## 需求编号补齐（2026-09-11，已部署20260911-259adf8）

1. 数据：需求公开编号、独立年度计数器及存量迁移；验收并发唯一、上海跨年、超过四位、删除不复用、编辑不变号和迁移幂等。
2. 显示：原需求列表、详情及保存成功提示原位替换内部ID，API读模型及本地原型生成/迁移同步；验收新旧需求显示一致，缺号不暴露UUID，关联和操作保持正常。
3. 验证：前后端类型检查、单测、构建、隔离API集成和浏览器需求流程；独立code-reviewer两阶段审查。用户随后明确授权部署，生产备份、迁移及健康检查通过。

验收结果：前端172/后端66单测、API101集成通过；原型45独立用例均有成功记录，成功提示补齐后专项1/1、真实API浏览器7/7通过；前后端构建及独立审查通过。详见 `it-project-console/docs/DEMAND-CODE-VERIFICATION.md`。
...
结果：类型/构建、API80/Web187单测、119+6隔离集成、发布9/Harness20通过；49原型和8真实浏览器用例均有通过记录，首轮冷启动超时与登录限流的复测详情见 it-project-console/docs/PROJECT-APPROVER-VERIFICATION.md。独立两阶段审查通过。用户随后明确授权部署，已发布20260914-1ed3b31并绑定姚泽攀；生产健康及权限校验通过，自动通知保持关闭。备份和验证边界见同一记录的生产章节。

## 2026-09-15 全局编辑本地预览执行
1. 在 project-card.vue、project-detail-drawer.vue 增加原型专用编辑入口，保留原操作。
2. 既有 project-fields 仅含创建/接单字段，progress-update-drawer 仅当前进度，均不能承载完整编辑；新增专职 project-edit-drawer，复用 ElDrawer、原型事务、人员资格工具和 useUnsavedForm，不新建应用壳。
3. 本地变更、核实、冲突与人员互斥验证，类型/单测/构建/浏览器验证及独立审查；启动现有前端交付预览。正式后端联动不在本轮。

## 2026-09-15 首次提出日期预览执行
1. 同步独立业务日期定义及范围；2. 原位补充卡片、项目详情、需求清单/详情和全局编辑；复用原型事务，校验及关联同步留痕；3. 单测覆盖有效、未知、未来、关联同步与原时间保留，类型/构建及浏览器验证后独立审查。本轮不改变月度统计、不接生产。

2026-09-15 精简预览：移除需求清单提交时间列，保留内部submittedAt与原统计；类型及原型界面检查。正式实现待补独立需求退回/重提审计。
2026-09-15 工程师核实预览：统一入口与保存权限，按已存储迁移状态和当前主责判断；覆盖主责、协作、无关人员、核实与转交后的权限回收，运行单测、构建及浏览器验证，独立审查。不接生产。

## 2026-09-15 正式全局编辑整改执行
A. 数据与后端：prisma/schema.prisma和新增迁移保存需求首次日期、项目描述/业务负责人/核实状态；projects/project-edit-schemas.ts、project-edit-service.ts、project-routes.ts提供版本及幂等保护的完整编辑接口；workspace/read-model.ts输出字段。验收：真实数据库权限正反例、迁移与核实/转交后回收、关联日期、并发及审计通过。
B. 需求流转：demands/demand-schemas.ts、demand-materials.ts、demand-service.ts及评审服务支持首次日期，补独立流转审计；project-service.ts立项继承需求日期。验收：退回重提保留业务日期，独立事件保留前后状态、操作人和发生时间。
C. 前端原位接入：project-card.vue、project-detail-drawer.vue、project-edit-drawer.vue、live-project-service.ts复用既有交互正式调用API；需求录入沿用既有表单增加日期。验收：两种运行模式保存/失败/冲突/权限变化正常；保留所有既有日常交互。
D. 联动与验收：复用既有项目人员、待办、负载与通知分发规则，必要时修正业务负责人关联；新增API隔离集成与前端回归；API/Web类型、单测、构建、隔离集成、浏览器及code-reviewer两阶段审查。生产上线单独授权。
实现结果：正式schema/read-model/编辑接口、需求独立审计及前端接入完成；已新增清空业务负责人后不回退旧人、实际验收通知收件人验证。API单测106、Web单测248、完整API隔离149项通过；真实浏览器7组12项通过，原型50项均有通过记录（冷加载/截图超时复测见验收记录）。类型、构建及独立两阶段审查通过，未部署生产。
## 2026-09-15 历史交付与负责人显示修正

1. 原位修正项目卡片/详情业务负责人字段；浏览器验证直接创建、有原提出人和清空三种显示。
2. 新增独立历史交付补录API，采用完整编辑权限、日期校验、版本与幂等事务，清理待验收并记录实际日期及操作审计；隔离集成覆盖权限、重放、日期、关闭状态、历史通知。
3. 完整编辑原抽屉新增历史交付对话框，保留草稿保护，原型及正式模式行为一致；类型、单测、构建、浏览器及独立审查验证。
## 2026-09-15 自动记录日常需求首次提出日期

1. 移除需求表单日期输入、校验及客户端字段；保留列表/详情日期显示和历史项目编辑。
2. 业务API严格拒绝日期改写，首次成功提交按服务器上海日期赋值，草稿/退回重提/失败/重复提交保持正确。
3. 原型与正式模式对齐；测试自动记录、上海日期边界、重提保留、篡改拒绝及原日期历史编辑，完成类型/构建、集成、浏览器和独立审查。
