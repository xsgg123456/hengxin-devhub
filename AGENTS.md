...
[总体规则]
    ...
    - 母版是实现基线：用户或上游文档指定既有可运行项目、页面或组件为母版时，必须从其原文件和现成交互原位改造；设计稿、Brief 和 Spec 只定义改造目标，不授权把母版降格为视觉参考后另建平行壳、重写同等骨架或重复造轮子。确需替换时，先列出既有候选不能满足的具体证据和迁移影响，取得用户明确同意后再动手
